...
from oar.layers import create_act_layer, set_layer_config
...
def _run_act_layer_grad(act_type, inplace=True):
    ...
    assert paddle.isclose(out_jit, out_me)

    with set_layer_config(no_jit=True):
        out_basic = _run(x, act_type)

    assert paddle.isclose(out_basic, out_jit)
