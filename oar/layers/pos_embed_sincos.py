...
def pixel_freq_bands(
    ...
    if linear_bands:
        bands = paddle.linspace(
            start=1.0, stop=max_freq / 2, num=num_bands, dtype=dtype
        )
    else:
        bands = 2 ** paddle.linspace(
            start=0, stop=math.log(max_freq, 2) - 1, num=num_bands, dtype=dtype
        )
...
def freq_bands(
    ...
    bands = 1.0 / temperature ** (
        paddle.arange(start=0, end=num_bands, step=step, dtype=dtype) / num_bands
    )
    return bands
...
def build_sincos2d_pos_embed(
    ...
    x = paddle.stack(
        x=paddle.meshgrid([paddle.arange(dtype=dtype, end=s) for s in feat_shape])
    ).flatten(start_axis=1)
    perm_0 = list(range(x.ndim))
    perm_0[0] = 1
    perm_0[1] = 0
    grid = x.transpose(perm=perm_0)
    pos2 = grid.unsqueeze(axis=-1) * bands.unsqueeze(axis=0)
    stack_dim = 2 if interleave_sin_cos else 1
    pos_emb = paddle.stack(
        x=[paddle.sin(x=pos2), paddle.cos(x=pos2)], axis=stack_dim
    ).flatten(start_axis=1)
    return pos_emb
...
def build_fourier_pos_embed(
    ...
    if bands is None:
        if in_pixels:
            ...
            bands = freq_bands(
                num_bands, temperature=temperature, step=1, dtype=dtype, device=device
            )
    else:
        if device is None:
            device = bands.place
    ...
    if in_pixels:
        t = [
            paddle.linspace(start=-1.0, stop=1.0, num=s, dtype=dtype)
            for s in feat_shape
        ]
    else:
        t = [paddle.arange(dtype=dtype, end=s) for s in feat_shape]
...
class FourierEmbed(nn.Layer):
    def __init__(
        ...
        self.register_buffer(
            name="bands", tensor=pixel_freq_bands(max_res, num_bands), persistable=False
        )
    ...
    def forward(self, x):
        ...
        perm_1[-1] = -2
        perm_1[-2] = -1
        emb = x.transpose(perm=perm_1).flatten(start_axis=len(feat_shape))
        batch_expand = (B,) + (-1,) * (x.ndim - 1)
        if self.keep_spatial:
            x = paddle.concat(
                x=[
                    x,
                    emb.unsqueeze(axis=0)
                    .expand(shape=batch_expand)
...
def rot(x):
    return paddle.stack(x=[-x[..., 1::2], x[..., ::2]], axis=-1).reshape(x.shape)


def apply_rot_embed(x: paddle.Tensor, sin_emb, cos_emb):
    if sin_emb.ndim == 3:
        return x * cos_emb.unsqueeze(axis=1).expand_as(y=x) + rot(
            x
        ) * sin_emb.unsqueeze(axis=1).expand_as(y=x)
...
def build_rotary_pos_embed(
    ...
    for x in feat_shape:
        num_spatial_dim *= x
    sin_emb = sin_emb.reshape(num_spatial_dim, -1).repeat_interleave(repeats=2, axis=-1)
    cos_emb = cos_emb.reshape(num_spatial_dim, -1).repeat_interleave(repeats=2, axis=-1)
    ...
    def __init__(
        ...
        if feat_shape is None:
            if in_pixels:
                ...
                print(bands)
            self.register_buffer(name="bands", tensor=bands, persistable=False)
            ...
            self.bands = None
            self.register_buffer(
                name="pos_embed_sin", tensor=emb_sin, persistable=False
            )
            self.register_buffer(
                name="pos_embed_cos", tensor=emb_cos, persistable=False
            )
    ...
    * https://blog.eleuther.ai/rotary-embeddings/
    ...
    def __init__(
        ...
        if feat_shape is None:
            ...
            self.register_buffer(
                name="pos_embed",
                tensor=paddle.concat(x=embeds, axis=-1),
                persistable=False,
            )
