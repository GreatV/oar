...
class ChannelAttn(nn.Layer):
    ...
    def forward(self, x):
        x_avg = self.fc2(self.act(self.fc1(x.mean(axis=(2, 3), keepdim=True))))
        x_max = self.fc2(self.act(self.fc1(x.amax(axis=(2, 3), keepdim=True))))
...
class LightChannelAttn(ChannelAttn):
    ...
    def forward(self, x):
        x_pool = 0.5 * x.mean(axis=(2, 3), keepdim=True) + 0.5 * x.amax(
            axis=(2, 3), keepdim=True
        )
        x_attn = self.fc2(self.act(self.fc1(x_pool)))
        return x * F.sigmoid(x=x_attn)
...
class SpatialAttn(nn.Layer):
    ...
    def forward(self, x):
        x_attn = paddle.concat(
            x=[x.mean(axis=1, keepdim=True), x.amax(axis=1, keepdim=True)], axis=1
        )
...
class LightSpatialAttn(nn.Layer):
    ...
    def forward(self, x):
        x_attn = 0.5 * x.mean(axis=1, keepdim=True) + 0.5 * x.amax(axis=1, keepdim=True)
        x_attn = self.conv(x_attn)
        return x * self.gate(x_attn)
