...
def adaptive_avgmax_pool2d(x, output_size: _int_tuple_2_t = 1):
    x_avg = F.adaptive_avg_pool2d(x=x, output_size=output_size)
    x_max = F.adaptive_max_pool2d(x=x, output_size=output_size)
...
def adaptive_catavgmax_pool2d(x, output_size: _int_tuple_2_t = 1):
    x_avg = F.adaptive_avg_pool2d(x=x, output_size=output_size)
    x_max = F.adaptive_max_pool2d(x=x, output_size=output_size)
    return paddle.concat(x=(x_avg, x_max), axis=1)
...
class FastAdaptiveAvgPool(nn.Layer):
    ...
    def forward(self, x):
        return x.mean(axis=self.dim, keepdim=not self.flatten)


class FastAdaptiveMaxPool(nn.Layer):
    def __init__(self, flatten: bool = False, input_fmt: str = "NCHW"):
    ...
    def forward(self, x):
        return x.amax(axis=self.dim, keepdim=not self.flatten)


class FastAdaptiveAvgMaxPool(nn.Layer):
    def __init__(self, flatten: bool = False, input_fmt: str = "NCHW"):
    ...
    def forward(self, x):
        x_avg = x.mean(axis=self.dim, keepdim=not self.flatten)
        x_max = x.amax(axis=self.dim, keepdim=not self.flatten)
        return 0.5 * x_avg + 0.5 * x_max


class FastAdaptiveCatAvgMaxPool(nn.Layer):
    def __init__(self, flatten: bool = False, input_fmt: str = "NCHW"):
    ...
    def forward(self, x):
        x_avg = x.mean(axis=self.dim_reduce, keepdim=not self.flatten)
        x_max = x.amax(axis=self.dim_reduce, keepdim=not self.flatten)
        return paddle.concat((x_avg, x_max), axis=self.dim_cat)
...
class SelectAdaptivePool2d(nn.Layer):
    ...
    def __init__(
        ...
        if not pool_type:
            ...
            self.flatten = nn.Flatten(start_axis=1) if flatten else nn.Identity()
