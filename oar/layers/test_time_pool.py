...
class TestTimePoolHead(nn.Layer):
    def __init__(self, base, original_pool=7):
        ...
        if isinstance(base_fc, nn.Conv2D):
            ...
            self.fc = nn.Conv2D(
                ...
                out_channels=self.base.num_classes,
                kernel_size=1,
                bias_attr=True,
            )
            paddle.assign(
                base_fc.weight.data.view(self.fc.weight.shape),
                output=self.fc.weight.data,
            )
            paddle.assign(
                base_fc.bias.data.view(self.fc.bias.shape), output=self.fc.bias.data
            )
        self.base.reset_classifier(0)

    def forward(self, x):
        x = self.base.forward_features(x)
        x = F.avg_pool2d(kernel_size=self.original_pool, stride=1, x=x, exclusive=False)
        x = self.fc(x)
