...
class RegularGridInterpolator:
    ...
    def __init__(self, points, values):
        ...
        assert isinstance(self.values, paddle.Tensor)
        self.ms = list(self.values.shape)
        self.n = len(self.points)
        assert len(self.ms) == self.n
        for i, p in enumerate(self.points):
            assert isinstance(p, paddle.Tensor)
    ...
    def __call__(self, points_to_interp):
        ...
        for p, x in zip(self.points, points_to_interp):
            idx_right = paddle.bucketize(x=x, sorted_sequence=p)
            idx_right[idx_right >= p.shape[0]] = p.shape[0] - 1
            idx_left = (idx_right - 1).clip(min=0, max=p.shape[0] - 1)
        ...
        for indexer in product([0, 1], repeat=self.n):
            ...
            numerator += self.values[as_s] * paddle.prod(x=paddle.stack(x=bs_s), axis=0)
        denominator = paddle.prod(x=paddle.stack(x=overalls), axis=0)
