...
class RotAttentionPool2d(nn.Layer):
    ...
    def __init__(
        ...
        self.qkv = nn.Linear(
            in_features=in_features, out_features=embed_dim * 3, bias_attr=qkv_bias
        )
        self.proj = nn.Linear(in_features=embed_dim, out_features=out_features)
        ...
        init_Constant = init.Constant(value=0.0)
        init_Constant(self.qkv.bias)
...
class AttentionPool2d(nn.Layer):
    ...
    def __init__(
        ...
        out_30 = paddle.create_parameter(
            shape=paddle.zeros(shape=[spatial_dim + 1, in_features]).shape,
            dtype=paddle.zeros(shape=[spatial_dim + 1, in_features]).numpy().dtype,
            default_initializer=init.Assign(
                paddle.zeros(shape=[spatial_dim + 1, in_features])
            ),
        )
        out_30.stop_gradient = False
        self.pos_embed = out_30
    ...
    def forward(self, x):
        ...
        perm_4[-2] = -1
        perm_4[-1] = -2
        attn = q @ x.transpose(perm=perm_4) * self.scale
        attn = F.softmax(attn, axis=-1)
        x = attn @ v
        perm_5 = list(range(x.ndim))
        perm_5[1] = 2
        perm_5[2] = 1
        x = x.transpose(perm=perm_5).reshape(B, N + 1, -1)
        x = self.proj(x)
        return x[:, 0]
