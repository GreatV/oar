...
class PatchEmbed(paddle.nn.Layer):
    ...
    def forward(self, x):
        ...
        if self.dynamic_img_pad:
            ...
            x = _FUNCTIONAL_PAD(pad=(0, pad_w, 0, pad_h), x=x)
        x = self.proj(x)
        if self.flatten:
...
def resample_patch_embed(
    ...
    def resize(x_np, _new_size):
        x_tf = paddle.to_tensor(data=x_np)[None, None, ...]
        x_upsampled = paddle.nn.functional.interpolate(
    ...
    resize_mat_pinv = paddle.to_tensor(
        data=np.linalg.pinv(resize_mat.T), place=patch_embed.place
    )
    ...
    patch_embed = patch_embed.astype(dtype="float32")
