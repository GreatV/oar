...
class ToTensor:
    ...
    def __call__(self, pil_img):
        ...
        if np_img.ndim < 3:
            np_img = np.expand_dims(np_img, axis=-1)
        np_img = np.rollaxis(np_img, 2)
        return paddle.to_tensor(data=np_img).to(dtype=self.dtype)
...
if has_interpolation_mode:
    _torch_interpolation_to_str = {
        paddle.vision.transforms.functional.InterpolationMode.NEAREST: "nearest",
        paddle.vision.transforms.functional.InterpolationMode.BILINEAR: "bilinear",
        paddle.vision.transforms.functional.InterpolationMode.BICUBIC: "bicubic",
        paddle.vision.transforms.functional.InterpolationMode.BOX: "box",
        paddle.vision.transforms.functional.InterpolationMode.HAMMING: "hamming",
        paddle.vision.transforms.functional.InterpolationMode.LANCZOS: "lanczos",
    }
...
class RandomResizedCropAndInterpolation:
    ...
    def __call__(self, img):
        ...
        return paddle.vision.transforms.functional.resized_crop(
...
def center_crop_or_pad(
    img: paddle.Tensor, output_size: List[int], fill=0
) -> paddle.Tensor:
    ...
    _, image_height, image_width = paddle.vision.transforms.functional.get_dimensions(
        img
    )
    ...
    if crop_width > image_width or crop_height > image_height:
        ...
        img = paddle.vision.transforms.functional.pad(img, padding_ltrb, fill=fill)
        _, image_height, image_width = (
            paddle.vision.transforms.functional.get_dimensions(img)
        )
        if crop_width == image_width and crop_height == image_height:
            return img
    crop_top = int(round((image_height - crop_height) / 2.0))
    crop_left = int(round((image_width - crop_width) / 2.0))
    return paddle.vision.transforms.functional.crop(
        img, crop_top, crop_left, crop_height, crop_width
    )


class CenterCropOrPad(paddle.nn.Layer):
...
class ResizeKeepRatio:
    ...
    def __call__(self, img):
        ...
        img = paddle.vision.transforms.functional.resize(img, size, self.interpolation)
        return img
