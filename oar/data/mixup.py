...
def one_hot(x, num_classes, on_value=1.0, off_value=0.0):
    x = x.astype(dtype="int64").view(-1, 1)
    return paddle.full(
        shape=(x.shape[0], num_classes), fill_value=off_value
    ).put_along_axis_(axis=1, indices=x, values=on_value)
...
def mixup_target(target, num_classes, lam=1.0, smoothing=0.0):
    ...
    y2 = one_hot(
        target.flip(axis=0), num_classes, on_value=on_value, off_value=off_value
    )
...
class Mixup:
    ...
    def _mix_elem(self, x):
        ...
        return paddle.to_tensor(data=lam_batch, dtype=x.dtype, place=x.place).unsqueeze(
            axis=1
        )
    ...
    def _mix_batch(self, x):
        ...
        if use_cutmix:
            ...
            x[:, :, yl:yh, xl:xh] = x.flip(axis=0)[:, :, yl:yh, xl:xh]
        else:
            x_flipped = x.flip(axis=0).multiply_(y=paddle.to_tensor(1.0 - lam))
            x.multiply_(y=paddle.to_tensor(lam)).add_(y=paddle.to_tensor(x_flipped))
        return lam
...
class FastCollateMixup(Mixup):
    ...
    def _mix_elem_collate(self, output, batch, half=False):
        ...
        for i in range(num_elem):
            ...
            output[i] += paddle.to_tensor(data=mixed.astype(np.uint8))
        if half:
            lam_batch = np.concatenate((lam_batch, np.ones(num_elem)))
        return paddle.to_tensor(data=lam_batch).unsqueeze(axis=1)
    ...
    def _mix_pair_collate(self, output, batch):
        ...
        for i in range(batch_size // 2):
            ...
            output[i] += paddle.to_tensor(data=mixed_i.astype(np.uint8))
            output[j] += paddle.to_tensor(data=mixed_j.astype(np.uint8))
        lam_batch = np.concatenate((lam_batch, lam_batch[::-1]))
        return paddle.to_tensor(data=lam_batch).unsqueeze(axis=1)
    ...
    def __call__(self, batch, _=None):
        ...
        if half:
            batch_size //= 2
        output = paddle.zeros(shape=(batch_size, *batch[0][0].shape), dtype="uint8")
        ...
        target = paddle.to_tensor(data=[b[1] for b in batch], dtype="int64")
        target = mixup_target(target, self.num_classes, lam, self.label_smoothing)
