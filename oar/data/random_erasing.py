...
def _get_pixels(per_pixel, rand_color, patch_size, dtype="float32", device="cuda"):
    if per_pixel:
        return paddle.empty(shape=patch_size, dtype=dtype).normal_()
    elif rand_color:
        return paddle.empty(shape=(patch_size[0], 1, 1), dtype=dtype).normal_()
    else:
