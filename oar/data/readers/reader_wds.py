...
def _load_info(root, basename='info'):
    ...
    try:
        ...
        pass
    _logger.warning(
        f'Dataset info file not found at {info_json} or {info_yaml}. Error: {err_str}. Falling back to provided split and size arg.'
        )
    return {}
...
def _parse_split_info(split: str, info: Dict):
    ...
    def _info_convert(dict_info):
        return SplitInfo(num_samples=dict_info['num_samples'], filenames=
            tuple(dict_info['filenames']), shard_lengths=tuple(dict_info[
            'shard_lengths']), alt_label=dict_info.get('alt_label', ''),
            name=dict_info['name'])
    if 'tar' in split or '..' in split:
        ...
        if '::' not in split:
            ...
            if split_idx and 'splits' in info and split_parts[split_idx
                ] in info['splits']:
        ...
        if split_name:
            ...
            if not num_samples:
                _fc = {f: c for f, c in zip(split_info['filenames'],
                    split_info['shard_lengths'])}
            ...
            split_info = SplitInfo(name=split_name, num_samples=num_samples,
                filenames=split_filenames)
    else:
        if 'splits' not in info or split not in info['splits']:
            raise RuntimeError(
                f"split {split} not found in info ({info.get('splits', {}).keys()})"
                )
        split = split
...
def _decode_samples(data, image_key='jpg', image_format='RGB', target_key=
    'cls', alt_label='', handler=log_and_continue):
    """Decode samples with skip."""
    for sample in data:
        try:
            result = _decode(sample, image_key=image_key, image_format=
                image_format, target_key=target_key, alt_label=alt_label)
...
class ResampledShards2(paddle.io.IterableDataset):
    """An iterable dataset yielding a list of urls."""

    def __init__(self, urls, nshards=sys.maxsize, worker_seed=None,
        ...
        self.worker_seed = (pytorch_worker_seed if worker_seed is None else
            worker_seed)
...
class ReaderWds(Reader):
    ...
    def __init__(self, root, name, split, is_training=False, batch_size=
        None, repeats=0, seed=42, class_map=None, input_name='jpg',
        input_image='RGB', target_name='cls', target_image='',
        prefetch_size=None, shuffle_size=None):
        super().__init__()
        if wds is None:
            raise RuntimeError(
    ...
    def _lazy_init(self):
        ...
        if self.worker_info is None:
            ...
            self.global_num_workers = self.dist_num_replicas * self.num_workers
            self.global_worker_id = (self.dist_rank * self.num_workers +
                self.worker_id)
        abs_shard_filenames = [os.path.join(self.root, f) for f in self.
            split_info.filenames]
        pipeline = [wds.SimpleShardList(abs_shard_filenames)]
        if self.is_training:
            pipeline.extend([detshuffle2(self.shard_shuffle_size, seed=self
                .common_seed, epoch=self.epoch_count), self.
                _split_by_node_and_worker, wds.tarfile_to_samples(handler=
                log_and_continue), wds.shuffle(self.sample_shuffle_size,
                rng=random.Random(self.worker_seed))])
        else:
            pipeline.extend([self._split_by_node_and_worker, wds.
                tarfile_to_samples(handler=log_and_continue)])
        pipeline.extend([partial(_decode_samples, image_key=self.image_key,
            image_format=self.image_format, alt_label=self.split_info.
            alt_label)])
        self.ds = wds.DataPipeline(*pipeline)

    def _split_by_node_and_worker(self, src):
        if self.global_num_workers > 1:
            for s in islice(src, self.global_worker_id, None, self.
                global_num_workers):
                yield s
        else:
            for s in src:
    ...
    def _num_samples_per_worker(self):
        num_worker_samples = self.num_samples / max(self.global_num_workers,
        ...
        if self.is_training and self.batch_size is not None:
            num_worker_samples = math.ceil(num_worker_samples / self.batch_size
                ) * self.batch_size
