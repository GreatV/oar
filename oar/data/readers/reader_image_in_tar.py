...
def extract_tarinfos(
    ...
    if os.path.exists(cache_path):
        ...
        with open(cache_path, "rb") as pf:
            info = pickle.load(pf)
        assert (
            len(info["tartrees"]) == num_tars
        ), "Cached tartree len doesn't match number of tarfiles"
    else:
    ...
    def _label_from_paths(*path, leaf_only=True):
        ...
        return (
            path.split(os.path.sep)[-1] if leaf_only else path.replace(os.path.sep, "_")
        )
    ...
    samples_and_targets = [
        (s, class_name_to_idx[l])
        for s, l in zip(samples, labels)
        if l in class_name_to_idx
    ]
    if sort:
        samples_and_targets = sorted(
            samples_and_targets, key=lambda k: natural_key(k[0][0].path)
        )
...
class ReaderImageInTar(Reader):
    ...
    def __getitem__(self, index):
        ...
        if self.cache_tarfiles:
            cache_state = (
                self.tar_state if self.root_is_tar else self.tar_state[parent_fn]
            )
            tf = cache_state.tf
        if tf is None:
        ...
        if child_ti is not None:
            ctf = (
                cache_state.children[child_ti.name].tf if self.cache_tarfiles else None
            )
            if ctf is None:
