...
def _extract_tarinfo(tf: tarfile.TarFile, parent_info: Dict, extensions:
    Set[str]):
    sample_count = 0
    for i, ti in enumerate(tf):
        ...
        if ext == '.tar':
            with tarfile.open(fileobj=tf.extractfile(ti), mode='r|') as ctf:
                child_info = dict(name=ti.name, path=os.path.join(
                    parent_info['path'], name), ti=ti, children=[], samples=[])
                sample_count += _extract_tarinfo(ctf, child_info,
...
def extract_tarinfos(root, class_name_to_idx: Optional[Dict]=None,
    cache_tarinfo: Optional[bool]=None, extensions: Optional[Union[List,
    Tuple, Set]]=None, sort: bool=True):
    extensions = get_img_extensions(as_set=True) if not extensions else set(
        extensions)
    root_is_tar = False
    ...
    if os.path.exists(cache_path):
        ...
        with open(cache_path, 'rb') as pf:
            info = pickle.load(pf)
        assert len(info['tartrees']
            ) == num_tars, "Cached tartree len doesn't match number of tarfiles"
    else:
        for i, fn in enumerate(tar_filenames):
            path = '' if root_is_tar else os.path.splitext(os.path.basename(fn)
                )[0]
            with tarfile.open(fn, mode='r|') as tf:
                parent_info = dict(name=os.path.relpath(fn, root), path=
                    path, ti=None, children=[], samples=[])
                num_samples = _extract_tarinfo(tf, parent_info, extensions=
                    extensions)
    ...
    def _label_from_paths(*path, leaf_only=True):
        ...
        return path.split(os.path.sep)[-1] if leaf_only else path.replace(os
            .path.sep, '_')
    ...
    for parent_info in info['tartrees']:
        ...
        for child_info in parent_info['children']:
            ...
            if child_added:
                tar_state.children[child_info['name']] = TarState(ti=
                    child_info['ti'])
    ...
    samples_and_targets = [(s, class_name_to_idx[l]) for s, l in zip(
        samples, labels) if l in class_name_to_idx]
    if sort:
        samples_and_targets = sorted(samples_and_targets, key=lambda k:
            natural_key(k[0][0].path))
    samples, targets = zip(*samples_and_targets)
    samples = np.array(samples)
    targets = np.array(targets)
    _logger.info(
        f'Finished processing {len(samples)} samples across {len(tarfiles)} tar files.'
        )
...
class ReaderImageInTar(Reader):
    ...
    def __init__(self, root, class_map='', cache_tarfiles=True,
        cache_tarinfo=None):
        super().__init__()
        ...
        self.root = root
        self.samples, self.targets, self.class_name_to_idx, tarfiles = (
            extract_tarinfos(self.root, class_name_to_idx=class_name_to_idx,
            cache_tarinfo=cache_tarinfo))
        self.class_idx_to_name = {v: k for k, v in self.class_name_to_idx.
            items()}
    ...
    def __getitem__(self, index):
        ...
        parent_abs = os.path.join(self.root, parent_fn
            ) if parent_fn else self.root
        tf = None
        cache_state = None
        if self.cache_tarfiles:
            cache_state = (self.tar_state if self.root_is_tar else self.
                tar_state[parent_fn])
            tf = cache_state.tf
        if tf is None:
        ...
        if child_ti is not None:
            ctf = cache_state.children[child_ti.name
                ].tf if self.cache_tarfiles else None
            if ctf is None:
