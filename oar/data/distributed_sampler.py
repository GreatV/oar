...
class OrderedDistributedSampler(paddle.io.Sampler):
...
class RepeatAugSampler(paddle.io.Sampler):
    ...
    def __iter__(self):
        ...
        if self.shuffle:
            indices = paddle.randperm(n=len(self.dataset))
        else:
            indices = paddle.arange(start=0, end=len(self.dataset))
        if isinstance(self.num_repeats, float) and not self.num_repeats.is_integer():
            ...
            indices = indices[
                paddle.to_tensor(
                    data=[int(i // self.num_repeats) for i in range(repeat_size)]
                )
            ]
        else:
            indices = paddle.repeat_interleave(
                x=indices, repeats=int(self.num_repeats), axis=0
            )
